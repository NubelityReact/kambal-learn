...
<div class="card">
    <div class="card-body">
        <form role="form" id="formServicio" data-toggle="validator" class="shake" autocomplete="off">
                        ...
                        <div class="row">
                            <div class="col-sm-2">
                                <div class="form-group">
                                    <input type="hidden" class="form-control" id="query_action" name="query_action" value="update" placeholder="Ingrese descripcion" required>
                                    <input type="hidden" class="form-control" id="idiservicio" name="idiservicio" placeholder="Ingrese descripcion" required>
            ...
            <button type="submit" id="form-submit" class="btn btn-success btn-lg pull-right "><span class="pe pe-diskette"></span> Guardar datos</button>
...
<script>
    $(document).ready(function () {
        $.ajax({
            ...
            data: "action=getServiciosEscolaresByID&idiservicio=" + idiservicio,
            success: function (text) {
                var servicio = text.data[0];
                $("#categoria").val(servicio.categoria);
                $("#descripcion").val(servicio.descripcion);
                $("#apply_discount").val(servicio.apply_discount);
                $("#precio").val(servicio.precio);
                $("#descuento").val(servicio.descuento);
                $("#estatus").val(servicio.estatus);
                $("#idiservicio").val(idiservicio);
            }
        });
    });
        ...
        if (r) {
            ...
            $.ajax({
                ...
                success: function (text) {
                    if (text == "success") {
                        formSuccess();
                        swalert("Exito!", 'Servicio actualizado correctamente', 'success');
                    } else {
                        formError();
