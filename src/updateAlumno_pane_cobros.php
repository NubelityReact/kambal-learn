<div class="card card-border-danger">
    <div class="card-header bg-white cfos"> <h4 class="float-left">Historial de Pagos</h4> <h4 id="subtotal" class="float-right"></h4></div>
    <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="number" min="0" class="form-control" id="recargo" name="recargo" placeholder="Enter recargo" required>
                                <div class="help-block with-errors text-danger"></div>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="number" min="0" class="form-control" id="total" name="total" placeholder="Enter total" required>
                                <div class="help-block with-errors text-danger"></div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function getMatriculaAlumnoByID(idialumno) {
        $.ajax({
            type: "GET",
            url: "dataConect/API.php",
            data: "action=getMatriculaAlumnoByID&idialumno=" + idialumno,
            success: function (text) {
                //console.log(text.data[0].matricula);
                var matricula = text.data[0].matricula;
                getCobrosPendientesToAlumnoById(matricula);
            }
        });
    }
    function getPartidasPendientesByIdiAlumno(idialumno) {
        ...
        $.ajax({
            ...
            success: function (text) {
                var date = text.data;
                var txt = "";
                txt += '<div> <span class="float-right"><a href="CxC2.php?idialumno=' + <?php echo $_GET['idialumno']; ?> + '" class="btn btn-success">Caja <i class="pe-7s-cart"></i></a></span><br><br><br></div>';
                ...
                for (x in date) {
                    var payment = '<a href="core_referenced_payment.php?idialumno=' + date[x].idialumno + '&idiventa_as_servicio=' + date[x].idiventa_as_servicio + '" target="_blank"><button type="button" id="payment" class="btn btn-warning btn-sm" data-toggle="tooltip" title="Imprimir Ficha de deposito"><i class="pe-7s-piggy pe-va"></i></button></a>';
                    txt += '<tr>';
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                document.getElementById("tableAdeudosByAlumno").innerHTML = "No se ha generado ningún Adeudo";
            }
        });
    }
</script>
<script>
    function getSubTotalOfServiciosByAlumno2(idialumno) {
        $.ajax({
            type: "GET",
            url: "dataConect/pagos.php",
            ...
            success: function (text) {
                //console.log(text);
                var date = text.data;
                //console.log("Subtotal: " + date[0].subtotal);
                $("#subtotal").append("<strong> Subtotal: </strong> $" + date[0].subtotal + " MXN");
                var txt = "";
                //console.log(date);
...
<script>
    //getSubTotalOfServiciosByAlumno subtotal Deuda total: $500
    function getSubTotalOfServiciosByAlumno(idialumno) {
        $.ajax({
            ...
            data: "action=getSubTotalOfServiciosByAlumno&idialumno=" + idialumno,
            success: function (text) {
                //console.log(text.data[0].subtotal);
                if (text.data[0].subtotal === null) {
                    $("#subtotal").html('Deuda total sin recargos: $ 0');
                } else {
                    $("#subtotal").html('Deuda total sin recargos: $' + text.data[0].subtotal);
                }
            }
        });
    }
