...
function submitForm() {
    ...
    $.ajax({
        ...
        success: function (text) {
            ...
            if (success == 1) {
                swalert('Mensaje', 'El grupo se creo correctamente', 'success');
                formSuccess(text);
...
$(document).on("click", ".fill", function () {
    ...
    var idinivel = $("#idinivel option:selected").text();
    ...
    var n = idinivel.substr(0, 1);
    var c = carrara.substr(0, 1);
    var t = turno.substr(0, 1);
    clave = n + grado + resultado + ciclo + t;
...
function sumarb() {
    ...
    $.ajax({
        ...
        success: function (text) {
            ...
            txt += '<thead class="table-primary text-light"> <tr><th>#</th><th>Grado</th><th>Alumnos</th><th>Grupo</th><th>Turno</th><th>Ciclo</th><th>Estatus</th><th>Clave</th></tr> </thead>';
            for (x in date) {
                var a = parseInt(x);
                txt += '<tr>';
                txt += "<td>" + (a + 1) + "</td>";
