        ...
        <div class="row">
            <div class="col-sm-2">
                <div class="form-group">
                    <label for="nivel">Seleccione el Plantel</label>
                    <div id="divPlantel"></div>
                    <div class="help-block with-errors text-danger"></div>
                </div>
            </div>
            <div class="col-sm-2">
                <div class="form-group">
                    <label for="nivel">Seleccione la oferta</label>
                    <div id="divCarrera">
            ...
            <div class="col-sm-1">
                <div class="form-group">
                    <label for="grado">Busqueda</label><br>
                    <span><a class="btn btn-sm btn-primary text-light" onclick="getAlumnoByCarreraAndGradoAndPlantel()">Buscar <i class="pe-7s-search"></i></a></span>
                    <div class="help-block with-errors text-danger"></div>
                </div>
            </div>
            <div class="col-sm-2">
                <label for="grado" class="">Agregar nuevo alumno</label><br>
                <span class=""><a href="core_alumno_addAlumno.php" class="btn btn-success btn-sm">Nuevo alumno <i class="pe-7s-add-user"></i></a></span>
            </div>
        </div>
    ...
    function getcarrera_idicampus() {
        ...
        $.ajax({
            ...
            success: function (text) {
                ...
                for (x in date) {
                    txt += '<option value="' + date[x].idicarrera + '">' + date[x].Descripcion + ' ' + date[x].nombre + '</option>';
                }
                txt += "</select>";
                $("#divCarrera").html(txt);
            }
...
<script>
    function getAlumnoByCarreraAndGradoAndPlantel() {
        ...
        $.ajax({
            type: "GET",
            url: "dataConect/API.php",
            data: "action=getAlumnoByCarreraAndGradoAndPlantel&idicampus=" + idicampus + "&idicarrera=" + idicarrera,
            success: function (text) {
                var date = text.data;
                var txt = "";
